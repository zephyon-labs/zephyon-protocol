...
export type SolanaSubmissionResult =
  | Readonly<{ outcome: "accepted"; submittedAt: string }>
  | Readonly<{ outcome: "settled"; settledAt: string; slot: string; confirmationStatus: string }>;

export type SolanaChainObservation =
  | Readonly<{ status: "missing"; observedAt: string; slot?: string; confirmationStatus?: string }>
  | Readonly<{ status: "pending"; observedAt: string; slot?: string; confirmationStatus?: string }>
  | Readonly<{ status: "settled"; observedAt: string; settledAt: string; slot: string; confirmationStatus: string }>
  | Readonly<{ status: "failed"; observedAt: string; failedAt: string; slot?: string; errorCode: string }>;
...
export class CanonicalSolanaPaymentRailAdapter implements CanonicalPaymentRailAdapter<CanonicalSolanaPreparedSubmission, SolanaRailEvidence> {
  ...
  async submit(prepared: CanonicalSolanaPreparedSubmission, context: RailOperationContext): Promise<SubmissionOutcome<SolanaRailEvidence>> {
    ...
    try {
      ...
      if (result.outcome === "settled") return Object.freeze({ outcome: "settled", providerReference, reconciliationReference, settledAt: result.settledAt, evidence: evidence(prepared, { result: "settled", slot: result.slot, confirmationStatus: result.confirmationStatus }) });
      return Object.freeze({ outcome: "accepted", providerReference, reconciliationReference, submittedAt: result.submittedAt, evidence: evidence(prepared, { result: "accepted" }) });
    } catch (error) {
  ...
  async reconcile(request: ReconciliationRequest, context: RailOperationContext): Promise<ReconciliationOutcome<SolanaRailEvidence>> {
    ...
    const base = { signature, cluster: this.transport.cluster } as const;
...
function evidencePayload(payload: { signature: string; cluster: string; rawAmount?: string; destination?: string; mint?: string; programId?: string; receiptPda?: string }, result: Record<string, string>): SolanaRailEvidence {
  return createRailEvidence({ type: "solana.settlement", version: 1, data: { cluster: payload.cluster, signature: payload.signature, ...(payload.rawAmount ? { rawAmount: payload.rawAmount } : {}), ...(payload.destination ? { destination: payload.destination } : {}), ...(payload.mint ? { mint: payload.mint } : {}), ...(payload.programId ? { programId: payload.programId } : {}), ...(payload.receiptPda ? { receiptPda: payload.receiptPda } : {}), ...result } }) as SolanaRailEvidence;
}
