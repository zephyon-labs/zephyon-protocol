// scripts/smoke_test_runtime_resilience_.ts
...
import { IdentityEngine, type IdentityService } from "../src/identity";
import { PolicyEngine, type PolicyService } from "../src/policy";
import { RiskEngine, type RiskService } from "../src/risk";
import {
...
async function main() {
  ...
  const requestId = `runtime-solana-devnet-${crypto.randomUUID()}`;
  ...
  const resilience = new ResilienceEngine({
    metadata: {
      script: "smoke_test_runtime_solana_devnet",
      rail: "solana",
      environment: "devnet",
    },
  });
  ...
  const resilientResult = await resilience.execute({
    ...
    failureFallback: {
      ...
      category: "settlement",
      retryable: true,
      suggestedAction: "Retry Solana runtime execution if infrastructure failed.",
      metadata: {
        rail: "solana",
        environment: "devnet",
      },
    ...
    operation: () =>
      runtime.execute({
        ...
        metadata: {
          memo: "Runtime Solana devnet smoke test with resilience wrapper",
        },
      }),
  });
  ...
  console.log("");
  console.log("========================================");
  console.log(" RESILIENT RUNTIME SOLANA DEVNET TEST");
  console.log("========================================");
  console.log("");

  console.log("Resilience Result:", resilientResult.ok ? "ok" : "failed");
  console.log("Resilience Context:");
  console.log(JSON.stringify(resilientResult.context, null, 2));
  console.log("");

  if (!resilientResult.ok) {
    console.log("Failure:");
    console.log(JSON.stringify(resilientResult.failure, null, 2));
    console.log("");

    console.log("Recovery Decision:");
    console.log(JSON.stringify(resilientResult.recoveryDecision, null, 2));
    console.log("");

    console.log("Retry Decision:");
    console.log(JSON.stringify(resilientResult.retryDecision, null, 2));
    console.log("");
  }

  if (resilientResult.ok) {
    const result = resilientResult.value;
    ...
    console.log(
      "Signature:",
      result.orchestration?.transaction.blockchain?.signature ?? "N/A",
    );
    console.log("");
  }
